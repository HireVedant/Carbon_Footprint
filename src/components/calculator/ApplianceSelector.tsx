  ...
  return (
    <div className="surface-matte space-y-4 p-6 rounded-2xl">
      <div className="flex items-center gap-3 pb-4 border-b border-border">
        ...
        <div>
          <h3 className="text-lg font-semibold text-text-primary">Appliances</h3>
          <p className="text-xs text-dark-500">BEE Star Rating adjusts energy consumption and emission accuracy.</p>
        </div>
      </div>
      ...
      {availableToAdd.length > 0 && (
        <div>
          <p className="text-[10px] uppercase tracking-wider text-dark-500 font-semibold mb-2">Add Appliance</p>
          <div className="grid grid-cols-2 gap-2">
            {availableToAdd.map(([key, cat]) => (
              <button
                ...
                className="flex items-center gap-2 p-3 border border-dashed border-border hover:border-primary-400 rounded-xl text-xs text-dark-500 hover:text-primary-600 transition-all"
              >
...
const MemoizedApplianceRow = React.memo(({
  ...
  return (
    <div className="bg-surface border border-dark-100 rounded-xl p-4 space-y-3 group">
      ...
      <div className="grid grid-cols-2 gap-3">
        {/* Star Rating */}
        <div>
          <label className="block text-[11px] font-medium text-dark-500 mb-1">BEE Star Rating</label>
        ...
        {/* Daily Hours */}
        <div>
          <label className="block text-[11px] font-medium text-dark-500 mb-1">Daily Usage (hours)</label>
          <input
            type="number"
            min={0.5}
            max={24}
            ...
            className="w-full bg-surface border border-border focus:border-primary-500 rounded-lg px-3 py-2 text-text-primary text-sm outline-none transition-all"
          />
        </div>
      </div>
      ...
      {/* Info pill */}
      {rating && (
        <div className="flex items-center justify-between bg-panel border border-dark-100 rounded-lg px-3 py-2 text-xs">
